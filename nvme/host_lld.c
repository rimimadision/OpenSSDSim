...
unsigned int check_nvme_cc_ams()
{
	NVME_STATUS_REG nvmeReg;

	nvmeReg.dword = IO_READ32(NVME_STATUS_REG_ADDR);
	
	return (unsigned int)nvmeReg.ccAms;
}
...
{
//	xil_printf("get nvme cmd 4.1.1\n");
	NVME_CMD_FIFO_REG nvmeReg;
	
	nvmeReg.dword = IO_READ32(NVME_CMD_FIFO_REG_ADDR);
	// xil_printf("cmdValid is %x, qID is %x, cmdSlotTag is %x, cmdSeqNum is %x, \r\n",nvmeReg.cmdValid,nvmeReg.qID,nvmeReg.cmdSlotTag,nvmeReg.cmdSeqNum);
	// xil_printf("qID is %x\r\n",nvmeReg.qID);
	// xil_printf("cmdSlotTag is %x\r\n",nvmeReg.cmdSlotTag);
	// xil_printf("cmdSeqNum is %x\r\n",nvmeReg.cmdSeqNum);
		
	if(nvmeReg.cmdValid == 1)
	{
//		xil_printf("get nvme cmd 4.1.2\n");
		//xil_printf("cmdValid is %x, qID is %x, cmdSlotTag is %x, cmdSeqNum is %x, \r\n",nvmeReg.cmdValid,nvmeReg.qID,nvmeReg.cmdSlotTag,nvmeReg.cmdSeqNum);
		unsigned int addr;
		unsigned int idx;
		*qID = nvmeReg.qID;
...
//	xil_printf("get nvme cmd 4.1.3\n");
	return (unsigned int)nvmeReg.cmdValid;
}

void set_auto_nvme_cpl(unsigned int cmdSlotTag, unsigned int specific, unsigned int statusFieldWord, unsigned int nvme_cpl_dw0)
{
	...
	nvmeReg.dword[1] = nvme_cpl_dw0;

	//xil_printf("set nvme cpl, cnme_cpl_dw1 = %d \n",nvmeReg.dword[1]);
...
void set_io_sq(unsigned int ioSqIdx, unsigned int valid, unsigned int cqVector, unsigned int qSzie, unsigned int pcieBaseAddrL, unsigned int pcieBaseAddrH ,unsigned int Qprio)
{
	...
	nvmeReg.sqPri=Qprio;//qxw add
	addr = NVME_IO_SQ_SET_REG_ADDR + (ioSqIdx * 8);
	IO_WRITE32(addr, nvmeReg.dword[0]);
	IO_WRITE32((addr + 4), nvmeReg.dword[1]);
}

void set_arb_weight(unsigned int dword11)
{
	NVME_ARB_WEIGHT_SET_REG nvmeReg;
	nvmeReg.dword=dword11;
	nvmeReg.valid=1;
	// unsigned int addr;
	// addr = ARBITRATION_PARAMETER_ADDR;
	IO_WRITE32(ARBITRATION_PARAMETER_ADDR, nvmeReg.dword);
}
