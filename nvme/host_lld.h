...
#define ARBITRATION_PARAMETER_ADDR (HOST_IP_ADDR + 0x2A0)
...
typedef struct _NVME_STATUS_REG
{
	union {
		...
		struct
		{
			...
			unsigned int ccAms : 3; 
			unsigned int reserved1 : 22;
		};
	};
} NVME_STATUS_REG;

//offset: 0x00000204, size: 1DW
...
//offset:0x000002A0,size=1DW
typedef struct _NVME_ARB_WEIGHT_SET_REG
{
	union {
		unsigned int dword;
		struct
		{
			unsigned int arbBurst:3;
			unsigned int valid:1;
...
} NVME_ARB_WEIGHT_SET_REG;

//offset: 0x00000300, size: 1DW
...
//offset: 0x00002000, size: 16 * 128 DW     128command
